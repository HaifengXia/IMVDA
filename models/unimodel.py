...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes) ## replace
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes) ## replace
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn3 = nn.BatchNorm2d(planes * 4) ## replace
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_clsses):
        self.inplanes = 64
...
def uniresnet50(num_classes):

    model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes)

    return model

def unimodel_init(model):
    key = '50_imagenet'
    url = models_urls[key]
    state_dict = maybe_download(key, url)
    model_dict = uniexpand_model_dict(model.state_dict(), state_dict)
    model.load_state_dict(model_dict, strict=True)

    return model


def uniexpand_model_dict(model_dict, state_dict):
